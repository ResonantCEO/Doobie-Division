...
### Object Storage
The application uses Replit Object Storage for persistent file storage:

- **Service**: Google Cloud Storage via Replit's object storage integration  
- **Product Images**: Uploaded via multer (memory storage) → saved to Object Storage at `{PRIVATE_OBJECT_DIR}/product-images/{uuid}.{ext}`, served via `/api/product-images/:filename`
- **Verification Photos**: Uploaded via multer (memory storage) → saved to Object Storage at `{PRIVATE_OBJECT_DIR}/verification-photos/{uuid}.{ext}`, served via `/api/verification-photos/:filename`
- **User Registration Photos**: Presigned URLs generated by backend, direct client-to-GCS upload via Uppy
- **ACL Policies**: Private photo access controlled by ownership metadata
- **Photo Types**: ID verification images and verification photos required for registration
- **Path Format**: Photos stored as `/objects/<uuid>` aliases mapping to `/<bucket>/.private/<uuid>`
- **Access Control**: Authenticated users can only access their own private photos via `/objects/*` route

Files:
- `server/objectStorage.ts`: Core object storage service with presigned URL generation and file retrieval
- `server/objectAcl.ts`: Access control policy management for private objects
- `client/src/components/ObjectUploader.tsx`: Uppy-based file upload component with progress tracking
...
- **Fixed Product Images Disappearing**: Migrated image uploads from local filesystem (ephemeral) to Replit Object Storage (persistent)
  ...
  - Verification photos: multer memory storage → GCS at `{PRIVATE_OBJECT_DIR}/verification-photos/`, served via `/api/verification-photos/:filename`
  - Removed legacy `/uploads` static file serving
  - Cleared 8 broken `/uploads/` URLs from database (files were already lost from production deployments)
- **Impact**: Product images now survive server restarts and redeployments
